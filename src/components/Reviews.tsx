...
function ReviewColumn({
  ...
  const [columnHeight, setCoulumnHeight] = useState(0);
  const duration = `${columnHeight * msPerPixel}ms`;

  useEffect(() => {
    if (!columnRef.current) return;
    const resizeObserver = new window.ResizeObserver(() => {
      setCoulumnHeight(columnRef.current?.offsetHeight ?? 0);
    });
  ...
  return (
    <div
      ref={columnRef}
      className={cn("animate-marquee space-x-0 py-4", className)}
...
function ReviewGrid() {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-x-0   h-32 top-0 bg-gradient-to-b from-slate-100"></div>
